...
void FormationControl::setupFormationControl()
{
   ...
   switchedToFormation = false;
    ...
    for (std::vector<int>::iterator it = _robotIDs.begin(); it != _robotIDs.end();it++) {
        ...
        setService(mainNodeHandle,*it,"/Option/traverse_flyto_safe/Flyto_safe/[0-9]*/tNeighbors/set",stringSrv);
        ...
        setService(mainNodeHandle,*it,"/Option/traverse_flyto_safe/Flyto_safe/[0-9]*/tVelocityInputTopic/set",stringSrv);

        stringSrv.request.input = options.tJoystickTopic->getValue();
        setService(mainNodeHandle,*it,"/Option/traverse_flyto_safe/Flyto_safe/[0-9]*/tJoystickTopic/set",stringSrv);

        stringSrv.request.input = "false";
        if(options.tDoYawFixPointControl->getValue()){
            stringSrv.request.input = "true";
        }
        setService(mainNodeHandle,*it,"/Option/traverse_flyto_safe/Flyto_safe/[0-9]*/tFormationDoYawFixPointControl/set",stringSrv);
        ...
        setService(mainNodeHandle,*it,"/Option/traverse_flyto_safe/Flyto_safe/[0-9]*/tYawFixPointInputTopic/set",stringSrv);

        stringSrv.request.input =  boost::lexical_cast<std::string>(options.tMaxYawRate->getValue());
        setService(mainNodeHandle,*it,"/Option/traverse_flyto_safe/Flyto_safe/[0-9]*/tMaxYawRate/set",stringSrv);
        ...
        setService(mainNodeHandle,*it,"/Option/traverse_flyto_safe/Flyto_safe/[0-9]*/tUsesHumanInput/set",stringSrv);

        setService(mainNodeHandle,*it,"/Option/traverse_flyto_safe/Flyto_safe/[0-9]*/tVelocityInputEnabled/set",stringSrv);

        stringSrv.request.input = boost::lexical_cast<std::string>(options.tFlyToDestinationRadius->getValue());
        setService(mainNodeHandle,*it,"/Option/traverse_flyto_safe/Flyto_safe/[0-9]*/tFlyToDestinationRadius/set",stringSrv);
        stringSrv.request.input = boost::lexical_cast<std::string>(options.tFlyToVelocity->getValue());
        setService(mainNodeHandle,*it,"/Option/traverse_flyto_safe/Flyto_safe/[0-9]*/tFlyToVelocity/set",stringSrv);
        ...
        setService(mainNodeHandle,*it,"/Option/traverse_flyto_safe/Flyto_safe/[0-9]*/tFlyToTopic/set",stringSrv);
...
void FormationControl::timeCB(const ros::TimerEvent&)
{
	...
	if (takeOffSignalReceived && !takeOffDone) 
	{
	  for (std::vector<int>::iterator it = _robotIDs.begin(); it != _robotIDs.end();it++) 
	  {
	      ...
	      srvTopic_ss << "/TeleKyb/flyto_safe_slave_" <<  *it << "/LiftLand";
	...
	if (now-timeAtTakeOff>10.0 && takeOffDone && !switchedToFormation) 
	{
	  ...
	  for (std::vector<int>::iterator it = _robotIDs.begin(); it != _robotIDs.end();it++) {
	      std::stringstream srvTopic_ss;
	      srvTopic_ss << "/TeleKyb/flyto_safe_slave_" <<  *it << "/SwtichToFormation";
	      ROS_INFO_STREAM(srvTopic_ss.str());
	      callService(mainNodeHandle,srvTopic_ss.str(),trigger_srv);
	  }
...
void FormationControl::joystickCB(const sensor_msgs::Joy::ConstPtr& msg)
{
    ...
    if (msg->buttons[5]) { //RB
        for (std::vector<int>::iterator it = _robotIDs.begin(); it != _robotIDs.end();it++) {
            std::stringstream srvTopic_ss;
            srvTopic_ss << "/TeleKyb/flyto_safe_slave_" <<  *it << "/Emergency";
    ...
    if (msg->buttons[0]) { // LB
        for (std::vector<int>::iterator it = _robotIDs.begin(); it != _robotIDs.end();it++) {
            ...
            if(behaviorRequest.response.behaviorName == "tk_behavior/Ground"){
                std::stringstream srvTopic_ss;
                srvTopic_ss << "/TeleKyb/flyto_safe_slave_" <<  *it << "/ToggleMotors";
                ROS_INFO_STREAM(srvTopic_ss.str());
                callService(mainNodeHandle,srvTopic_ss.str(),trigger_srv);
            }
        }
    ...
    if (msg->buttons[4]) { // A
        for (std::vector<int>::iterator it = _robotIDs.begin(); it != _robotIDs.end();it++) {
            std::stringstream srvTopic_ss;
            srvTopic_ss << "/TeleKyb/flyto_safe_slave_" <<  *it << "/ToggleYaw";
            ROS_INFO_STREAM(srvTopic_ss.str());
            callService(mainNodeHandle,srvTopic_ss.str(),trigger_srv);
        }
    }
